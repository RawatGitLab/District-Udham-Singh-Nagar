...
import { Search, MapPin, Minimize2, Maximize2, AlertTriangle, BarChart3, ChevronDown, ChevronUp, Database, Wifi, Cpu, Activity, Table } from "lucide-react";
...
export default function AttributeTable({
  ...
  return (
    <div className="w-[450px] border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 flex flex-col h-full shrink-0 shadow-sm font-sans transition-all duration-300">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 rounded-md hover:bg-slate-50 dark:hover:bg-slate-800 transition duration-150 border border-slate-200 dark:border-slate-700 flex items-center justify-center"
            >
            ...
            <button
              ...
              className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 rounded-md hover:bg-slate-50 dark:hover:bg-slate-800 transition duration-150 border border-transparent"
            >
            ...
            {currentLayer && (
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full text-xs pl-9 pr-4 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-indigo-500 focus:bg-white dark:focus:bg-slate-800 transition"
                />
              </div>
            )}
          </>
      ...
      {/* Stats Counter Bar */}
      {currentLayer && (
        <div className="px-4 py-2 bg-slate-50/50 dark:bg-slate-850/50 border-b border-slate-200 dark:border-slate-800 flex justify-between text-[11px] items-center text-slate-500 dark:text-slate-400 font-medium font-mono">
      ...
      <div className="flex-1 overflow-auto min-h-0 bg-white dark:bg-slate-900 relative">
        {!currentLayer ? (
          ...
          <table className="w-full text-left text-xs border-collapse font-sans">
            <thead>
              <tr className="bg-slate-100/60 dark:bg-slate-800 sticky top-0 border-b border-slate-200 dark:border-slate-700 shadow-sm z-10 text-slate-600 dark:text-slate-300 font-bold leading-tight select-none">
                ...
                return (
                  <tr
                    ...
                    className={`group cursor-pointer text-slate-700 dark:text-slate-200 transition duration-150 ${
                      isSelected 
                        ? "bg-indigo-50 dark:bg-indigo-950/70 hover:bg-indigo-100 dark:hover:bg-indigo-900/80 text-indigo-900 dark:text-indigo-200 border-l-2 border-l-indigo-600 border-b border-indigo-200 dark:border-indigo-800" 
                        : "hover:bg-slate-50 dark:hover:bg-slate-800/60 border-l-2 border-l-transparent"
                    }`}
                  >
                    <td className="py-2.5 px-3 text-center border-r border-slate-100 dark:border-slate-800 font-mono text-[10px] text-slate-400 dark:text-slate-500 bg-slate-50/50 dark:bg-slate-800/40">
                      {index + 1}
                    </td>
